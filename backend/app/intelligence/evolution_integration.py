...
class EvolutionIntegration:
    ...
    def discover_context_patterns(
        ...
        for record in records:
            ...
            # Build pattern key
            pattern_key = f"{trade_data.get('symbol')}_" \
    ...
    def generate_context_recommendations(
        ...
        strong_patterns = [p for p in patterns if p.win_rate > 0.65 and p.confidence > 0.7]
        ...
        weak_patterns = [p for p in patterns if p.win_rate < 0.45 and p.confidence > 0.5]
    ...
    def detect_context_weaknesses(
        ...
        for record in records:
            ...
            if (record.content["context"].get("regime") == context.regime.value and
                record.content["context"].get("session") == context.session.value):
